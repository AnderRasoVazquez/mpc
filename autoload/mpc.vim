...
function! mpc#DisplayPlaylist()
  ...
  for track in playlist
    let output = track.position . " "
          ...
          \ . track.album
          \ . track.title
    let l:artist_decorator = substitute(track.artist, '\s\+$', '', '')
    if(playlist[0].position == track.position)
      execute "normal! 1GdGI" . l:artist_decorator
      call append(line('$'), output)
    else
      if (l:previous_artist != track.artist)
        call append(line('$'), l:artist_decorator)
      endif
      call append(line('$'), output)
    endif
    let l:previous_artist = track.artist
  endfor
  execute('normal zM')
endfunction
